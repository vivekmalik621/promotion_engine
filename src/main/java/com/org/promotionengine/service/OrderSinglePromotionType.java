...
/**
 * OrderSinglePromotionType processes Single type promotion and calculates the order price
 * 
 * @author Vivek Malik
 *
 */
...
public class OrderSinglePromotionType implements OrderPromotionType {
	...
	public BigDecimal process(OrderPromotion orderPromotion) {
		...
		if (requestedQuantity < promotion.getQuantity()) {
			orderPrice = orderPrice.add(product.getUnitPrice().multiply(new BigDecimal(requestedQuantity)));
