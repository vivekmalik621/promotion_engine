	 ...
	 @Autowired
	    ...
	    public BigDecimal process(final OrderPromotion orderPromotion) {
	        ...
	        if (null == appliedComboExcludedItemsMap.get(requestedSku)) {
	            ...
	            //create comboPromotionMap with the all SKU's applicable for the given Combo Promotion
	            promotionRepository.findByComboReferenceId(promotion.getComboReferenceId()).stream()
	                .forEach(comboPromo -> {
	                    comboPromotionMap
	                        .put(comboPromo.getProduct().getSku(), comboPromo.getQuantity());
	                });

	            if (OrderRequest.isComboValid(orderRequest, comboPromotionMap)) {
	                orderPrice = orderPrice.add(promotion.getPrice());
	                //if comboPromotion is valid on the requested item, then maintain the excluded items map to capture other sku's for the Combo Promotion
	                promotionRepository.findByComboReferenceId(promotion.getComboReferenceId()).stream()
	                    .forEach(comboPromo -> {
	                        appliedComboExcludedItemsMap
	                            .put(comboPromo.getProduct().getSku(), comboPromo.getQuantity());
	                    });
	            } else {
